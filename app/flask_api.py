...
def select_dataset():
    ...
    transformed_data_filename = f"transformed_{dataset_name}.csv"
...
def filter_years():
    ...
    try:
        # selected_dataset = pd.read_csv(download_file_path/selected_dataset_filename)  # Load the DataFrame from the file
        selected_dataset = pd.read_csv(f'{download_file_path}/{selected_dataset_filename}')
    ...
    if len(year_data) == 2:
        ...
        filtered_year_df.to_csv(f'{download_file_path}/{selected_dataset_filename}', index=False)
    elif len(year_data) == 1:
        filtered_year_df = filtered_data.filter_by_year(year_data[0], None)
        filtered_year_df.to_csv(f'{download_file_path}/{selected_dataset_filename}.csv', index=False)
    else:
...
def filter_zipcodes():
    ...
    filtered_df_zipcodes.to_csv(f'{download_file_path}/{selected_dataset_filename}.csv', index=False)
    return jsonify(filtered_df_zipcodes.to_dict(orient='records'))

@app.route('/download_datafile/', methods = ["GET"])
def download_data():
    global selected_dataset
    selected_dataset_filename = session.get('selected_dataset')
    print(session)
    if selected_dataset_filename is None:
        return jsonify({'error': 'No dataset selected'}), 400
    try:
        if os.path.isfile(f'{download_file_path}/{selected_dataset_filename}'):
            print('download_file_path', download_file_path, '----------------')
            print('selected_dataset_filename', selected_dataset_filename, '+++++++++++++')
            return send_from_directory(directory=download_file_path, filename=f"{selected_dataset_filename}", as_attachment=True)
